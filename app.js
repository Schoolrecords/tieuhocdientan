/*
============================================================================
 app.js — Logic chính của Hệ thống Hồ sơ số (template tiểu học)
============================================================================
 ...
 KHÔNG chứa (giữ inline trong index.html):
   ...
   • const API_URL — đọc từ window.HSS_CFG.appsScriptUrl (config.js)
...
/* ===== Phần 1: JS chính HSS (render, Admin, Setup, callGAS) ===== */
  // 🔗 const API_URL được đọc từ config.js (window.HSS_CFG.appsScriptUrl) ở
  //    index.html. app.js (cũng classic script) tham chiếu trực tiếp.
  ...
  const faqs = [
    // ── Nhóm 1: Hồ sơ số (HSS) ──
    {q:'Hệ thống Hồ sơ số gồm những gì?', a:'Hệ thống gồm 109 hồ sơ được tổ chức thành 11 nhóm theo chức năng: Hiệu trưởng, Phó hiệu trưởng, Tổ chuyên môn, Hành chính, Kế toán, Đảng, Đội, Ban ĐDCMHS, Hồ sơ CBGVNV, KĐCL và ĐBCL. Nhấn nút "📋 Tổng quan Danh mục HSS" để xem toàn bộ.'},
    ...
    // ── Nhóm 3: QL Chất lượng (QLCL) ──
    {q:'📊 Hệ thống QL Chất lượng (QLCL) là gì?', a:'Là <b>hệ thống riêng</b> để nhập kết quả học tập của học sinh theo TT 27/2020: <b>sổ điểm, sổ năng lực-phẩm chất, nhận xét, xếp loại HS</b>, sau đó <b>xuất Học bạ Word</b> đúng mẫu và <b>đồng bộ tự động</b> lên CSDL ngành (MOET). Mở từ menu chính → nhấn ô <b>"📊 QL Chất lượng"</b> (nền xanh đậm) — tab mới mở ra. Khác KĐCL: KĐCL là kiểm định cả trường, còn QLCL là kết quả học tập của từng HS.'},
    {q:'📊 Vào QLCL cần đăng nhập sao? Mật khẩu lấy ở đâu?', a:'Mỗi GV/NV/Admin có 1 <b>tài khoản riêng</b> được Hiệu trưởng/PHT cấp qua Zalo nhóm trường. Mỗi máy/điện thoại chỉ cần đăng nhập <b>1 lần đầu</b> — các lần sau tự động bỏ qua. Nếu quên: liên hệ HT/PHT để cấp lại (HT vào QLCL → Phân quyền CBGV → đặt lại mật khẩu).'},
    {q:'📊 Vai trò GV và Admin trong QLCL khác nhau thế nào?', a:'<b>Giáo viên:</b> Chỉ thấy + sửa điểm lớp mình chủ nhiệm; xuất học bạ lớp mình; xem thống kê lớp mình.<br><b>Admin (HT/PHT):</b> Thấy toàn trường, khoá/mở kỳ điểm (chốt điểm), xuất học bạ tất cả khối, đồng bộ MOET, quản lý chữ ký + dấu trường, phân quyền CBGV.'},
    {q:'📊 Thông tư 27 quy định nhập điểm như thế nào?', a:'Theo TT 27/2020/TT-BGDĐT:<br>• <b>Môn học:</b> dùng <b>T / H / C</b> (Hoàn thành Tốt / Hoàn thành / Chưa hoàn thành). Riêng Toán và Tiếng Việt có thêm <b>điểm KTĐK 1-10</b> ở cuối kỳ.<br>• <b>Năng lực &amp; Phẩm chất:</b> dùng <b>T / Đ / C</b> (Tốt / Đạt / Cần cố gắng). 8 năng lực + 5 phẩm chất.<br>• <b>Xếp loại HS:</b> HTXS (Hoàn thành Xuất sắc), HTT (Hoàn thành Tốt), HT (Hoàn thành), CHT (Chưa hoàn thành) — hệ thống tự tính theo Điều 9 TT 27.<br>QLCL áp dụng đúng quy định này, không dùng 4 mức của TT 22 cũ.'},
    {q:'📊 Nhập điểm có cần bấm Lưu không?', a:'<b>Không cần</b>. Hệ thống <b>tự động lưu</b> sau mỗi 2 giây kể từ khi bạn rời ô. Có thông báo ✓ "Đã lưu" nhỏ ở góc trên màn hình. Vì vậy bạn có thể yên tâm gõ → tab → gõ tiếp, không lo mất dữ liệu.'},
    {q:'📊 Lỡ nhập sai khi kỳ đã khoá thì làm sao?', a:'GV không thể sửa khi kỳ đã khoá (sidebar hiện chấm đỏ 🔒). Báo cho Hiệu trưởng/PHT → HT vào QLCL → mục <b>Kỳ đánh giá</b> → bấm <b>Mở khoá</b> kỳ cần sửa → GV sửa xong → HT khoá lại. Việc khoá-mở được ghi log để truy vết.'},
    {q:'📊 Xuất Học bạ Word có cần Microsoft Word không?', a:'Khi <b>xuất</b> thì không — hệ thống tạo file <code>.docx</code> hợp chuẩn ngay trên trình duyệt. Sau đó mở file bằng Word, Google Docs, hoặc WPS đều được. Để in/ký, mở bằng Microsoft Word cho định dạng chuẩn nhất.'},
    {q:'📊 Tại sao học bạ thiếu chữ ký HT / dấu / GVCN?', a:'Vì chưa upload ảnh chữ ký. Vào QLCL → sidebar mục <b>"✍️ Ảnh chữ ký &amp; dấu"</b> → upload 3 ảnh <b>PNG nền trong suốt</b>: (1) Chữ ký HT, (2) Dấu trường, (3) Chữ ký GVCN. Nên dùng nền trong (không trắng) để chèn lên dòng kẻ học bạ đẹp hơn.'},
    {q:'📊 Đồng bộ MOET tự động hoạt động ra sao?', a:'Quy trình:<br>(1) Cài Chrome Extension "HSS Sync MOET" (file <code>hss-sync-extension/</code> kèm template) — 1 lần duy nhất.<br>(2) Mở tab CSDL ngành (csdl.moet.gov.vn), đăng nhập thủ công (có CAPTCHA).<br>(3) Sang tab QLCL → nhấn "🔄 Đồng bộ CSDL ngành" → chọn Khối + Kỳ → bấm "Đẩy lên".<br>Extension sẽ: lấy kết quả từ QLCL → tạo file Excel 35 cột chuẩn MOET → tìm tab CSDL → tự set dropdown → tự upload → tự click "Tải lên". Tất cả mất ~30 giây.'},
    {q:'📊 Có cần cài thêm phần mềm để dùng QLCL không?', a:'<b>Để nhập điểm + xuất học bạ:</b> Không cần — chỉ cần trình duyệt (Chrome/Edge/Safari).<br><b>Để đồng bộ MOET tự động:</b> Cần cài Chrome Extension "HSS Sync MOET" (1 lần, miễn phí). Nếu không cài cũng được — vẫn xuất Excel theo mẫu MOET rồi upload thủ công lên csdl.moet.gov.vn.'},
    // ── Nhóm 4: Tổng quát ──
    ...
    {q:'Xem trên điện thoại có được không?', a:'Hoàn toàn được. Giao diện tự điều chỉnh cho màn hình nhỏ, menu thu gọn thành nút ☰ ở góc phải. Tất cả tính năng — kể cả nhập điểm QLCL và xuất học bạ — đều hoạt động trên điện thoại. Mẹo: thêm trang vào màn hình chính (Chrome menu ⋮ → "Thêm vào màn hình chính") để dùng như app.'},
  ...
  function _buildCatalogPrintHTML(forWord){
    ...
    return '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Danh mục Hồ sơ số — ' + escapeHtml(schoolName) + '</title>'
      ...
      + '<div style="font-style:italic;margin-bottom:8pt">' + (((window.HSS_CFG && window.HSS_CFG.xa) || '').replace(/^Xã\s+/i,'') ? (((window.HSS_CFG.xa).replace(/^Xã\s+/i,'')) + ', ') : '') + dateStr + '</div>'
...
'</div></body></html>';
  ...
  function renderAbout(){
    ...
    var name = cfg.name || (window.HSS_CFG && window.HSS_CFG.schoolName) || 'Trường Tiểu học';
    var addr = cfg.address || (window.HSS_CFG && window.HSS_CFG.diaChi) || '';
  ...
  function _buildStyledHSSheet(){
    ...
    var schoolName = cfg.name || ((window.HSS_CFG && window.HSS_CFG.schoolName) || 'TRƯỜNG TIỂU HỌC').toUpperCase();
    var schoolAddr = cfg.address || (window.HSS_CFG && window.HSS_CFG.diaChi) || '';
    ...
    if (hasData) {
      ...
      var demos = [
        [1, '1A', 'HS001', 'Nguyễn Văn A',  '15/03/2018', 'Nam', 'Kinh', '', 'Tỉnh ABC', '', 'Xã XYZ', 'Xóm 5', 'Xã XYZ, Tỉnh ABC', '0901234567', 'Nguyễn Văn B', '1985', 'Trần Thị C', '1987', 'Đang học'],
        [2, '1A', 'HS002', 'Trần Thị B',    '22/07/2018', 'Nữ',  'Kinh', '', 'Tỉnh ABC', '', 'Xã XYZ', 'Xóm 3', 'Xã XYZ, Tỉnh ABC', '0912345678', 'Trần Văn D', '1983', 'Lê Thị E',  '1986', 'Đang học'],
        [3, '1B', 'HS003', 'Lê Hoàng C',    '10/11/2018', 'Nam', 'Kinh', '', 'Tỉnh ABC', '', 'Xã XYZ', 'Xóm 7', 'Xã XYZ, Tỉnh ABC', '',           'Lê Văn F',   '1984', 'Phạm Thị G','1988', 'Đang học']
      ];
