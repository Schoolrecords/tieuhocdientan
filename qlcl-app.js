/*
============================================================================
 qlcl-app.js — Logic Quản lý Chất lượng Giáo dục Tiểu học (TT 27/2020)
============================================================================
 Đi cùng index.html (Hồ sơ số) trong cùng repo: dùng CHUNG 1 Sheet + 1 Apps
 Script bound. URL backend đọc từ config.js → window.HSS_CFG.appsScriptUrl.
...
// └──────────────────────────────────────────────────────────┘
var APP_VERSION='3.2';
// 🔗 URL Web App HSS — đọc từ config.js (window.HSS_CFG.appsScriptUrl).
//    Dùng chung với index.html: 1 Sheet, 1 Apps Script bound.
var DEFAULT_GAS = (window.HSS_CFG && window.HSS_CFG.appsScriptUrl) || '';

// 🧩 Extension ID HSS Sync (Chrome Extension đồng bộ CSDL ngành MOET).
//    Sau khi cài extension từ thư mục hss-sync-extension → chrome://extensions/
//    → copy ID → dán vào HSS_CFG.extId trong config.js.
var HSS_EXT_ID = (window.HSS_CFG && window.HSS_CFG.extId) || '';
...
function _hssWizardStepHTML(step){
  if(step===1){
    ...
    '<div style="font-size:13px;color:var(--slate2);line-height:1.7">'+
      ...
      '<div style="background:#f1f5f9;border:1px solid #cbd5e1;border-radius:6px;padding:10px;font-family:monospace;font-size:11.5px;color:#1e40af;margin:10px 0;word-break:break-all">…\\hss-sync-extension</div>'+
...
function _rptSchool(){return localStorage.getItem('school_name_full') || (window.HSS_CFG && window.HSS_CFG.schoolName) || 'Trường Tiểu học';}
function _rptXa(){return localStorage.getItem('school_xa') || (window.HSS_CFG && window.HSS_CFG.xa) || '';}
function _rptTinh(){return localStorage.getItem('school_tinh') || (window.HSS_CFG && window.HSS_CFG.tinh) || '';}
function _rptDiaChi(){return localStorage.getItem('school_addr') || (window.HSS_CFG && window.HSS_CFG.diaChi) || '';}
...
function openHB(idx){
  ...
  h+=_liInp('hb_noi_sinh',    'Nơi sinh',             llNoiSinh, 'VD: Bệnh viện Đa khoa tỉnh',      s.noi_sinh || '');
  h+=_liInp('hb_que_quan',    'Quê quán',             llQueQuan, 'VD: Xã ABC, tỉnh XYZ',            s.que_quan || '');
  h+=_liInp('hb_noi_o',       'Nơi ở hiện nay',       llNoiO,    'VD: Xóm/Thôn, xã ABC...',         s.cho_o || '');
...
function _buildHocBaData(s){
  ...
  return {
    ...
    truong: school,
    xa: xa,
    // xa_short: tên xã KHÔNG có prefix "Xã/Phường/Thị trấn" (template Word đã có
    // sẵn label "Xã " trước placeholder), dùng cho ô "(Phường, Thị trấn): Xã __"
    xa_short:  (xa || '').replace(/^(Xã|Phường|Thị trấn)\s+/i, ''),
    tinh: tinh,
    // tinh_short: tên tỉnh không có prefix "Tỉnh/Thành phố" (dự phòng nếu template
    // có label "Tỉnh " trước placeholder)
    tinh_short:(tinh || '').replace(/^(Tỉnh|Thành phố)\s+/i, ''),
    nam_hoc: nam_hoc,
    hieu_truong: ht,
    gvcn: gvcn,
...
function expBangCLWord(){
  ...
  html+='<td style="width:50%"><i>'+(_rptXa() ? _rptXa().replace(/^Xã\s+/i,'')+', ' : '')+dateStr+'</i><br><b>Hiệu trưởng</b><br><i style="color:#888;font-size:11pt">(Ký, ghi rõ họ tên và đóng dấu)</i><br><br><br><br><br><b>'+ht+'</b></td>';
  html+='</tr></table>';
