/**
 * =====================================================================================
 *  BACKEND CHUNG - HỆ THỐNG HỒ SƠ SỐ TRƯỜNG TIỂU HỌC (TEMPLATE)
 *  Tên trường + địa chỉ điền qua Admin → Thông tin trường (lưu sheet CauHinh).
...
// Backend chạy trên cùng Sheet HSS (9 tab nhập tay hoặc migrate từ Sheet QLCL nguồn).
...
// Để rỗng = chế độ MỞ (chỉ dùng tạm khi setup; có log cảnh báo).
// Vào Web → Admin → Bảo mật → đặt 2 mã thật → ghi vào Script Properties.
const AUTH_TOKEN_GV    = '';   // Mã GV    — sửa điểm/nhận xét. Đổi qua Admin → Bảo mật.
const AUTH_TOKEN_ADMIN = '';   // Mã Admin — Admin panel + KĐCL + cả mã GV. Đổi qua Admin → Bảo mật.
// ----------------------------------------------------------------------------
//
...
/**
 * =====================================================================================
 *  HỒ SƠ SỐ - TRƯỜNG TIỂU HỌC (TEMPLATE)
 *  Tên trường, địa chỉ, Hiệu trưởng… nhập qua Web → Admin → Thông tin trường
...
const SCHOOL_CONFIG = {
  name:           '',  // VD: 'Trường Tiểu học Diễn Tân' — nhập qua Admin → Thông tin trường
  address:        '',  // VD: 'Xã Quảng Châu, Tỉnh Nghệ An'
  phone:          '',
  email:          '',
...
const DATA_HSS = [
  ...
  ["105", "11.1.1. VB chỉ đạo ĐBCL của Sở GD&ĐT", "", "Phó HT phụ trách CM", "ĐBCL"],
...
// ============================================================================
const _DRIVE_ROOT_NAME = 'HoSoSo_TH';   // Có thể đổi tên cho rõ trường (vd 'HoSoSo_THDienTan')

/**
 * Bootstrap cấu trúc folder Drive theo namHoc, ID lưu vào Config.
 * Idempotent — tìm folder theo tên, không tồn tại mới tạo.
 *   <_DRIVE_ROOT_NAME>/{namHoc}/ChuKy/ChuKy_GVCN/
 *   <_DRIVE_ROOT_NAME>/{namHoc}/HocBa/
 */
 ...
 *   không phụ thuộc DSGV (DSGV có thể trống ở giai đoạn đầu).
...
//   <_DRIVE_ROOT_NAME>/{namhoc}/HocBa/{lop}/{ma_hs}_{ho_ten}/{*.docx, *.pdf}
// ============================================================================
...
// có thể nhập tay trên Sheet hoặc migrate từ Sheet QLCL nguồn qua
// migrateQlclFromExternal (nếu trường đã có Sheet QLCL chạy trước đó).
// ============================================================================
...
// ============================================================================
// MIGRATION — Copy 9 tab QLCL từ Sheet ngoài sang Sheet HSS hiện tại.
// Chỉ dùng khi trường đã có Sheet QLCL chạy trước và muốn gộp về 1 Sheet.
// Trường mới setup từ đầu KHÔNG cần chạy hàm này — bỏ qua.
// ============================================================================
/**
 * Cách chạy: Apps Script editor → chọn migrateQlclFromExternal → ▶ Run.
 * Hệ thống sẽ prompt nhập Sheet ID nguồn (Sheet QLCL cũ của trường).
 *
...
function migrateQlclFromExternal() {
  ...
  try {
    const r = ui.prompt(
      ...
      'Paste Sheet ID của Sheet QLCL nguồn (Sheet cũ của trường):\n\nLấy từ URL: docs.google.com/spreadsheets/d/<ID>/edit',
      ui.ButtonSet.OK_CANCEL
    );
